...
**Ответ.** 19 минут обучения `multilingual-e5-small` (118M) поднимают nDCG@10 на тесте с 0.802 до **0.859**. Это ровно уровень `multilingual-e5-large` (560M, 0.859): разрыв закрыт полностью, при этом задержка на CPU в 6.8 раза меньше, а индекс в 2.7 раза меньше. Рецепт: синтетические вопросы двух LLM, разметка обучающих пар моделью-учителем FRIDA и дистилляция её ранжирования.
...
- **Задача `RuLawRetrieval` для MTEB:** готова и проверена их же кодом — [`mteb_task/`](mteb_task/)
...
**Вопросы** генерируют две локальные LLM из разных семейств (llama.cpp, Q4_K_M), чтобы стиль обучающих вопросов не совпадал со стилем теста: train — Qwen3-8B (два прохода, второй просит вопросы про конкретные детали статьи), dev / test / tk_hard — YandexGPT-5-Lite-8B, каждый вопрос проверяет LLM-судья (Qwen3-8B).
...
Три стиля вопросов: `everyday` («Я купил бизнес, а там куча проблем: не всё имущество есть, и ещё долги, о которых мне не сказали. Можно ли как-то цену снизить?»), `search` («запрет на посредников при усыновлении»), `legal` («Каков порядок определения размера алиментов на нетрудоспособных совершеннолетних детей при отсутствии соглашения об уплате алиментов?»).
...
**Обобщение.** Прирост сохраняется и на отложенных статьях тех же кодексов, по которым обучающих вопросов не было вовсе (`unseen_articles`, +0.058 [+0.031; +0.086]). На статьях с обучающими вопросами он больше (`seen`, +0.080), но и там вопросы теста другие. То есть модель учит язык домена, а не запоминает конкретные статьи. На кодексах, которых не было в обучении (`unseen_codes`: СК и ЗоЗПП), прирост +0.026, и интервал задевает ноль — это честная граница метода. Там же самый маленький запас: для базовых моделей этот срез оказался самым лёгким (0.837 против 0.785 на `seen`), потому что СК и ЗоЗПП тематически обособлены.
...
Прирост дообучения здесь вдвое больше, чем на основном тесте: **+0.109** [+0.091; +0.128].
...
|---|---|---:|---:|
| Родные 30 вопросов | Hit@1 / MRR@5 | 0.960 / 0.980 | **0.960 / 0.980** |
| 74 вопроса по ТК из теста | Hit@1 / MRR@5 | 0.770 / 0.857 | **0.797 / 0.859** |
| Трудный tk_hard (716) | Hit@1 / MRR@5 | 0.591 / 0.688 | **0.721 / 0.793** |

На трудном наборе MRR@5 растёт на +0.105 [+0.084; +0.127], по бытовым вопросам на +0.128; на насыщенных наборах — паритет. Генерация ответов не изменилась (ссылки на верную статью 25/25, faithfulness 1.000), а отказы на вопросах без ответа стали 5/5 вместо 4/5. Замер «было/стало» делался **до** переключения модели — именно он и решает, стоит ли переключать. Изменения: [pull request #1](https://github.com/alekseevpavel04/tk-rf-rag/pull/1), прогоны в `eval/finetuned/`.
...
- **Учитель FRIDA вместо базовой e5-small.** У e5-small косинусы сжаты в узкий диапазон, поэтому правило «негатив должен быть ниже 0.95 от позитива» отбирало негатив только для 30% вопросов ([`mining_e5-small.json`](results/mining_e5-small.json)); у FRIDA шкала шире, и правило работает для 92% ([`teacher_train_llm_v12.json`](results/teacher_train_llm_v12.json)). Учитель же выбирает позитивный фрагмент внутри статьи и отбрасывает 5% вопросов, которые не связываются со своей статьёй, — это шум генерации.
- **Дистилляция ранжирования** (`DistillKLDivLoss` по 8 кандидатам, оценки FRIDA) добавляет +0.003 на dev поверх контрастного обучения и, что важнее, делает модель устойчивой к формату нарезки: dev в формате сервиса вырос с 0.8465 (этап E) до 0.8527 (этап F).
- **Второй проход генерации:** кривая обучения не выходила на плато, и ещё 7.6 тыс. вопросов дали +0.004 на dev.
- **Аугментация формата не понадобилась** — устойчивость к формату дал учитель.
...
|---|---:|---:|---:|---:|---:|---:|
| **e5-small-ru-law** | 118 | 384 | 20.2 | 23.2 | 15.7 | 0.859 |
| e5-base | 278 | 768 | 40.5 | 41.9 | 16.2 | 0.822 |
| USER2-base | 149 | 768 | 40.5 | 57.9 | 27.2 | 0.825 |
| bge-m3 | 568 | 1024 | 53.9 | 151.7 | 26.8 | 0.856 |
| e5-large | 560 | 1024 | 53.9 | 158.5 | 27.7 | 0.859 |
| FRIDA | 823 | 1536 | 80.9 | 270.2 | 61.4 | 0.878 |
| Qwen3-Emb-0.6B | 596 | 1024 | 53.9 | 340.0 | 66.9 | 0.843 |
...
Windows 11, Python 3.11, RTX 3070 8 ГБ, около 4 часов GPU на всё обучение.
...
python -m pytest                                         # 88 быстрых тестов; -m slow: ещё 12 на префиксы моделей
```

Каждый шаг идемпотентен: готовые файлы и обученные чекпоинты пропускаются. `scripts/run_eval.sh` — единственный источник всех чисел в `results/` и в этом README.
...
scripts/     генерация, сборка данных, абляции, этапы рецепта, финальная оценка, экспорт модели
mteb_task/   задача RuLawRetrieval для MTEB: класс, статистика, прогоны, текст PR
tests/       парсер, фрагменты, сплиты, метрики, фильтры, утечки, префиксы моделей, сэмплер, бутстрап
