...
На трудном наборе прирост MRR@5 равен +0.105 [+0.084; +0.127], по бытовым вопросам +0.128. Генерация на 30 вопросах не изменилась (ссылки на верную статью 25/25, faithfulness 1.000, ложных отказов нет), отказы на вопросах без ответа стали 5/5 вместо 4/5. Первая версия модели в сервисе проигрывала базовой, вторая выигрывает: это ровно тот случай, ради которого нужен замер «было/стало» до переключения. Изменения сервиса: [pull request #1](https://github.com/alekseevpavel04/tk-rf-rag/pull/1), таблицы и прогоны в `eval/finetuned/`.
