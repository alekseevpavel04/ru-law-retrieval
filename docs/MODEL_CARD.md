...
print(q @ d.T)
# эта модель:             [[0.539 0.191 0.415]]  ст. 81 уверенно первая
# intfloat/multilingual-e5-small: [[0.863 0.819 0.859]]  статьи почти не различаются
```

Абсолютные косинусы у дообученной модели ниже, чем у исходной e5 (шкала растянута обучением с MultipleNegativesRankingLoss). Значение имеет порядок: пороги, подобранные под исходную e5, к этой модели не переносятся.
